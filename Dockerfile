...
RUN apt-get update -qq && apt-get upgrade -y && apt-get install -y build-essential curl git python wget
...
RUN bundle install --deployment --without development test -j "$(nproc)"
...
CMD ["gosu app bundle exec /home/app/main.rb"]
